import { Tables } from "generated/enums/tables";
import { SupabaseClient } from "generated/supabase-client";
import { useQueryClient } from "react-query";
import type { UseMutationResult } from "hooks/use-mutation";
import { useMutation } from "hooks/use-mutation";

interface UseDeletePublicOptions {
    onError?: (error: Error) => void;
    onSuccess?: () => void;
}

const useDeletePublic = (
    options?: UseDeletePublicOptions
): UseMutationResult<void, Error, string> => {
    const { fromPublics } = SupabaseClient;
    const { onError, onSuccess } = options ?? {};
    const queryClient = useQueryClient();

    const deleteFn = async (id: string) => {
        const { error } = await fromPublics().delete().eq("id", id);

        if (error != null) {
            throw error;
        }
    };

    const result = useMutation<void, Error, string>({
        fn: deleteFn,
        onSuccess,
        onError,
        onSettled: () => {
            queryClient.invalidateQueries(Tables.Publics);
        },
    });

    return result;
};

export { useDeletePublic };
